...
def main():
    ...
    for index, build_url in enumerate(builds_urls, 1):
        try:
            ...
            dict_build["Name"] = soup_build.find(
                "h1", {"class": "pageTitle build__name"}
            ).text
            print(
                f"Extracting build {index} - {dict_build['Name']} at {build_url}"
            )
            dict_build["Build Link"] = build_url
            dict_build["Description"] = soup_build.find(
                "div", {"class": "markdown"}
            ).text.replace("\n", "")
            dict_build["Votes"] = soup_build.find(
                "a", {"class": "actionBox actionBox__vote"}
            ).text
            dict_build["Comments number"] = soup_build.find(
                "a", {"class": "actionBox__comments"}
            ).text
            dict_build["Author"] = (
                soup_build.find("div", {"class": "user"}).find("a").text
            )
            ...
            details_title = [
                x.text
                for x in soup_build.find_all("h4", {"class": "group__title"})
            ]
            details_value = [
                x.text
                for x in soup_build.find_all(
            ...
            table_components = soup_list.find(
                "table", {"class": "xs-col-12"}
            ).find_all("tr", {"class": "tr__product"})
            old_component_type = "TEST"
            count_component_type = 2
            for component in table_components:
                try:
                    component_type = component.find(
                        "td", {"class": "td__component"}
                    ).text.strip()
                    if component_type == "":
                        component_type = (
                            f"{old_component_type}_{count_component_type}"
                        )
                ...
                try:
                    ...
                    pass
                try:
                    component_final_price = (
                        component.find("td", {"class": "td__price"})
                        .text.strip()
                        .replace("Price", "")
                    )
                except Exception as e:
                ...
                try:
                    component_shop = (
                        component.find("td", {"class": "td__where"})
                        .find("a")["href"]
                        .split("/")[2]
                    )
                except Exception as e:
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Scraper pcpartpicker.com (builds)"
    )
    parser.add_argument(
        "--debug",
    ...
    parser.add_argument(
        ...
        default="Exports/list_builds_urls.txt",
    )
